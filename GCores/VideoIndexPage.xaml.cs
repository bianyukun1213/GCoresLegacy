...
namespace GCores
{
    ...
    public sealed partial class VideoIndexPage : Page
    {
        ...
        async void GetVideos(int pageNumber)
        {
            try
            {
                HttpClient hc = new HttpClient();
                HttpResponseMessage response = await hc.GetAsync(new Uri("http://www.g-cores.com/api/categories/8/originals?page=" + pageNumber + "&auth_exclusive=" + SplashPage.authExclusive));
                ...
                for (int i = 0; i < obj.GetNamedArray("results").Count; i++)
                {
                    ...
                    string media = obj.GetNamedArray("results")[i].GetObject().GetNamedObject("media").GetNamedArray("flv")[0].GetString();
